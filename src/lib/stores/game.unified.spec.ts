...
describe('統一エンジン: 練習/特定札の難易度・スコアリング', () => {
	it('練習モードは session.difficulty を持たない（=標準スコアリング）', async () => {
		...
		expect(get(gameStore.gameStore).session?.difficulty).toBeUndefined();
	});
	...
	it('ランダムモードは渡した難易度を session に保持する', async () => {
		...
		expect(get(gameStore.gameStore).session?.difficulty).toBe('beginner');
	});
});
...
describe('統一エンジン: 全文ひらがなで検証（hiraganaShort を使わない）', () => {
	it('練習モードは hiraganaShort を無視して全文を検証対象にする', async () => {
		...
		const validator = get(gameStore.gameStore).input.validator as InputValidator;
...
describe('統一エンジン: 完了とコンボ挙動', () => {
	it('全札を入力し終えるとセッションが終了する', async () => {
		...
		expect(get(gameStore.gameStore).session?.isActive).toBe(false);
	});
	...
	it('バックスペース（入力短縮）ではコンボが途切れない', async () => {
		...
		const comboBefore = get(gameStore.gameStore).statistics.currentCombo;
		expect(comboBefore).toBe(2);

		gameStore.updateInput('a'); // バックスペース相当（'ai' → 'a'）
		const comboAfter = get(gameStore.gameStore).statistics.currentCombo;
